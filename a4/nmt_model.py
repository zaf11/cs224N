...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.att_projection = nn.Linear(in_features=2*self.hidden_size, out_features=2*self.hidden_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded)#(src_len, batch, embedding_size)
        print("X.shape:", X.shape)
        print("source_lengths:", source_lengths)
        X = pack_padded_sequence(X, source_lengths)#压缩输入，只保留没有pad的部分
        print("X.shape:", len(X[0]), len(X[1]))
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)#enc_hiddens保留最后一层lstm每个timestamp的隐藏层状态。
        ...
        print("init_decoder_hidden:", init_decoder_hidden)
